...
async function handleClick(e, click_from_nested_iframe) { 
	...
	const isCtrlClick=(e.ctrlKey && ! e.shiftKey && !e.altKey &&! e.metaKey && e.type=="mousedown");
	if (isCtrlClick || click_from_nested_iframe) {
		prevDef(e);
	...
	function fadeFully(el, that){
		el.add(curtainRod).delay(200).fadeOut(
			1000, function(){
				...
				curtainRod.css({visibility:"hidden",display:""});
				curtainRod.remove();
				let isOnLastCurtain = (el[0]==this && --that.final_curtain==0);
				if (isOnLastCurtain) finishedCB();
			});
	}
} // end closeCurtains().
...
function postMessageHandler(e){ // window.postMessage comms
	const isMsgForWebEraser=( e.data?.type == "sfswe-iframe-click");
	if (isMsgForWebEraser) {
		if (gobj.iframe) {
...
function manimate(objs,[css_attr,target_val,suffix,delay],interval,noOf_subintervals,CB) { // CB is invoked once, at end.  $.animate max-ed out cpu for 30 secs or so.
	...
	setTimeout(()=>intervalId=setInterval(eppursimuove,subinterval,params), delay||0);
} 
...
function eppursimuove(ps) {try{
	...
	else requestAnimationFrame(tstamp=>ps.objs.css(ps.css_attr,ps.plotvals[ps.cnt[0]]+ps.suffix));  // width attribute
